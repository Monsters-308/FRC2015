...
public class Vision extends Subsystem {
	...
	public Vision() {
		...
		try {
			...
			camera = new AxisCamera("10.3.8.30");
	...
	public void process() {
		...
		if (numParticles > 0) {
			...
			// an exercise
			// for the reader. Note that the long and short side scores
			// expect a single tote and will not work for a stack of 2 or
			// more totes.
			// Modification of the code to accommodate 2 or more stacked
			// totes is left as an exercise for the reader.
			...
			if (isTote) {
				...
				NIVision.imaqDrawShapeOnImage(
						...
						new NIVision.Rect(
								...
								(int) Math.abs(particles.elementAt(0).BoundingRectBottom
										- particles.elementAt(0).BoundingRectTop),
								(int) Math.abs(particles.elementAt(0).BoundingRectRight
